...
echo "$($PSQL "select course from students inner join majors using(major_id) inner join majors_courses(major_id) inner join courses using(course_id) group by course having count(student_id) = 1 order by course")"
